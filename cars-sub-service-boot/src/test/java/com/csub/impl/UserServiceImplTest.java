...
class UserServiceImplTest {
    ...
    @Test
    void addUser() {
        Mockito.when(userDAO.addUser(user)).thenReturn(user.getId());
        long actual = userService.addUser(user);
    ...
    void addUserMustThrowServerException() {
        Mockito.when(userDAO.addUser(user)).thenReturn(0L);
    ...
    void checkIfEmailAlreadyExistsMustThrowServerException() {
        ...
        assertThatThrownBy(() -> userService.updateUser(userUpdate,userUpdate.getId()))
