...
public final class CarCriteriaBuilderManager {
    ...
    private static void setFilterInfo(CarSearchInfo info, CriteriaBuilder builder, Root<Car> root, List<Predicate> fieldPredicates) {
        fieldPredicates.add(builder.equal(root.get("carStatus").get("name"), "AVAILABLE"));
            ...
            for (String field : info.getFilter()) {
                ...
                if (filter.length == 2) {
                    switch (filter[0]) {
                        ...
                        case YEAR_FROM ->
                                fieldPredicates.add(builder.greaterThanOrEqualTo(root.get(YEAR), Integer.parseInt(filter[1])));
                        case YEAR_TO ->
                                fieldPredicates.add(builder.lessThanOrEqualTo(root.get(YEAR), Integer.parseInt(filter[1])));
                        case PRICE -> fieldPredicates.add(builder.equal(root.get(PRICE), Integer.parseInt(filter[1])));
                        case PRICE_FROM ->
                                fieldPredicates.add(builder.greaterThanOrEqualTo(root.get(PRICE), Integer.parseInt(filter[1])));
                        case PRICE_TO ->
                                fieldPredicates.add(builder.lessThanOrEqualTo(root.get(PRICE), Integer.parseInt(filter[1])));
                    }
                }
